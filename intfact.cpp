...
void* factorize(char* tuple, int stage, int& factor1, int& factor2, int& polarity) {
	...
	while (1) {
		fscanf(fp, "%c", &pp);
		fscanf(fe, "%c", &ee);
		if ((polarity == -1 || polarity == t) && stage == hit && pp == tuple[0] && ee == tuple[1]) {
			if (polarity == t) {
				polarity = t;
				//calculate
				int pk = pp - '0';
				...
				if (pk > ek) {
					...
					if (_ek == 0 || _last_ek == 0 || _pk == 0 || _last_pk == 0) {
						cout << "\nindeterminate at this stage\n";
						//mark down
					} else {
						cout << factor1 << "\t" << factor2 << "\t" << polarity << endl;
					}
				} else if (pk < ek) {
					if (_last_ek < _ek) {
						_last_ek += 10;
		...
		} else if (polarity == (1 - t) && stage == hit && pp == tuple[0] && ee == tuple[1]) {
			//mark down
			++hit;
		}
		t = 1 - t;
